#María Isabella Guerra Delgadillo
#Lancelot Coy Ponce
#Rodrigo Alexander Soto
...
# para que se vean todas las columnas sin los ...
pd.set_option("display.max_columns", None)
pd.set_option("display.max_rows", None)
pd.set_option("display.width", 1000)
pd.set_option("display.max_colwidth", None)

plt.style.use("seaborn-v0_8")  # Estilo general para todas las gráficas
...
print(f"{var1} -> media: {df[var1].mean(skipna=True):.2f}, "f"mediana: {df[var1].median(skipna=True):.2f}, "f"moda: {df[var1].mode(dropna=True).iloc[0] if not df[var1].mode(dropna=True).empty else None}")
print(f"{var2} -> media: {df[var2].mean(skipna=True):.2f}, "f"mediana: {df[var2].median(skipna=True):.2f}, "f"moda: {df[var2].mode(dropna=True).iloc[0] if not df[var2].mode(dropna=True).empty else None}")
...
sns.boxplot(y=df[var1], color="lightblue")
plt.title(f"Boxplot -{var1}", fontsize=14)
plt.ylabel(var1, fontsize=12)
plt.tight_layout(); plt.show()

# Histograma de Ingresos Mensuales
plt.figure(figsize=(6, 4))
sns.histplot(df[var1], bins=10, kde=True, color="skyblue")
plt.title(f"Histograma - {var1}", fontsize=14)
plt.xlabel("Valor", fontsize=12); plt.ylabel("Frecuencia", fontsize=12)
plt.tight_layout(); plt.show()

# Boxplot de Precio Membresía
plt.figure(figsize=(6, 4))
sns.boxplot(y=df[var2], color="salmon")
plt.title(f"Boxplot - {var2}", fontsize=14)
...
# Método del codo
plt.figure(figsize=(6, 4))
plt.plot(list(k_values), inertias, marker="o", color="teal")
plt.xticks(list(k_values))
plt.title("Método del codo (inertia vs k)", fontsize=14)
plt.xlabel("k", fontsize=12); plt.ylabel("Inertia", fontsize=12)
plt.tight_layout(); plt.show()
...
plt.xticks(list(k_values))
plt.title("Índice de Silueta vs k", fontsize=14)
plt.xlabel("k", fontsize=12); plt.ylabel("Silhouette", fontsize=12)
plt.tight_layout(); plt.show()
...
dist_df = pd.DataFrame(dist_centers, index=centers_df.index,
columns=centers_df.index)
...
# ¿Cómo determinaste el valor de k a usar?
print("\n2) Elección de k:")
...
# ¿Crees que los centros obtenidos son representativos de los datos? ¿Por qué? ¿Qué ocurriría si usaras un valor más alto o más bajo de k?
...
#¿Qué pasa con outliers en boxplot (según los boxplots)??
...
print("\n7) Estrategias de negocio por segmento:")
print("- Para centros con alto ingreso y membresía costosa: reforzar servicios premium, fidelización y marketing personalizado.")
print("- Para centros con bajo ingreso y membresía económica: optimizar costos operativos, campañas de captación masiva.")
print("- Para centros con tráfico medio y precios intermedios: diferenciarse por experiencia, convenios corporativos o clases especializadas.")
print("- Recomendación: usar los perfiles de cluster para diseñar estrategias de precios, ubicación y promoción adaptadas.")
...
try:
    ...
    for lab in np.unique(labels):
        idx = labels == lab
        plt.scatter(X2[idx, 0], X2[idx, 1], label=f"Cluster {lab}", s=60, alpha=0.7, color=palette[lab])
    plt.title("Clusters K-Means (PCA 2D)", fontsize=14)
    plt.xlabel("Componente 1", fontsize=12); plt.ylabel("Componente 2", fontsize=12)
    plt.legend()
    plt.tight_layout(); plt.show()
